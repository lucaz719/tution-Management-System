...
import { canReleaseAdmissionLogins, isInvoiceOverdue, oneYearEnrollmentWindow, recurringInvoiceType } from './billing-rules';
import { studentBillingSummary } from './student-billing-summary';
import { invoiceLineItems } from './invoice-document';
...
const enrollmentWindow = oneYearEnrollmentWindow(new Date('2026-08-31T12:15:00.000Z'));
assert.equal(enrollmentWindow.validFrom.toISOString(), '2026-08-31T12:15:00.000Z');
assert.equal(enrollmentWindow.validUntil.toISOString(), '2027-08-31T12:15:00.000Z');
...
assert.deepEqual(
  invoiceLineItems([{ label: 'Physics', amount: 2200 }, { label: 'Chemistry', amount: 2100 }], 'SUBJECT', 4300),
  [{ label: 'Physics', amount: 2200 }, { label: 'Chemistry', amount: 2100 }],
  'receipts preserve the original invoice lines',
);
assert.deepEqual(
  invoiceLineItems(null, 'ADMISSION', 1500),
  [{ label: 'One-time admission fee', amount: 1500 }],
  'legacy invoices receive a safe readable fallback',
);
