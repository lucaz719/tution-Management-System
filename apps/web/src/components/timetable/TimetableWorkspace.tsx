          ...
          <form className="timetable-form" onSubmit={submit} aria-busy={saving}>
            <div className="people-drawer-body">
              {!editingId ? <fieldset className="timetable-path"><legend>1. Choose where this class belongs</legend>
                ...
                <label htmlFor="class-course">Course <span>*</span><select id="class-course" required disabled={!form.gradeId} value={form.courseId} onChange={(event) => { const course = courses.find((item) => item.id === event.target.value); setForm((current) => ({ ...current, courseId: event.target.value, name: course?.gradeId ? `${course.gradeName} - Section A` : course?.name || '' })); }}><option value="">{form.gradeId ? 'Select course…' : 'Choose grade first'}</option>{formCourses.map((course) => <option key={course.id} value={course.id}>{course.name}</option>)}</select></label>
