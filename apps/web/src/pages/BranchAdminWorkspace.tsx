...
export function BranchResultsView() {
  ...
  const [maximum, setMaximum] = useState('100'); const [passMarks, setPassMarks] = useState('40'); const [deleteArmed, setDeleteArmed] = useState(false); const action = useAction();
  ...
  const publishEvent = () => { if (!selectedEvent) return; void action.run(async () => { await api.branchAdmin.publishResultDefinition(selectedEvent.id); await load(branchId); }, 'All available drafts published to students and parents.'); };
  ...
  return <Page title="Branch results" description="Create an event, use its protected student roster CSV, then preview the standard printable report.">
    ...
    <Card hoverable={false}><h2 style={{ fontSize: 18 }}>2. Import and manage results</h2>{data.resultDefinitions.length ? <div style={form}><label style={label} htmlFor="result-event">Result event<select id="result-event" style={field} value={selectedId} onChange={(event) => { setSelectedId(event.target.value); setCsvRows([]); setCsvError(''); setDeleteArmed(false); }}>{data.resultDefinitions.map((item: any) => <option key={item.id} value={item.id}>{item.title} · {item.subject} · {new Date(item.testDate).toLocaleDateString()}</option>)}</select></label>{selectedEvent ? <div className="result-event-summary"><div><StatusBadge variant={selectedEvent.isOpen ? 'success' : 'warning'}>{selectedEvent.isOpen ? 'Entry open' : 'Entry closed'}</StatusBadge><p>Manage the event lifecycle without changing its grade, class, or subject identity.</p></div><div className="result-event-actions"><Button variant="outline" onClick={() => void downloadTemplate(selectedId)}>Download CSV</Button><Button variant="outline" onClick={() => void printReport(selectedId)}>Preview report</Button><Button variant="outline" onClick={toggleEvent}>{selectedEvent.isOpen ? 'Close entry' : 'Reopen entry'}</Button><Button onClick={publishEvent}>Publish drafts</Button></div></div> : null}<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(160px,1fr))', gap: 12 }}><label style={label} htmlFor="result-maximum">Full marks<input id="result-maximum" inputMode="decimal" style={field} value={maximum} onChange={(event) => setMaximum(event.target.value)} /></label><label style={label} htmlFor="result-pass">Pass marks<input id="result-pass" inputMode="decimal" style={field} value={passMarks} onChange={(event) => setPassMarks(event.target.value)} /></label></div><label style={label} htmlFor="result-csv">Completed CSV<input id="result-csv" type="file" accept=".csv,text/csv" style={field} onChange={(event) => void chooseCsv(event.target.files?.[0])} aria-invalid={Boolean(csvError) || undefined} aria-describedby={csvError ? 'result-csv-error' : 'result-csv-help'} /><small id="result-csv-help" style={{ color: 'var(--text-muted)' }}>Do not rename columns or change student IDs. Maximum 500 rows.</small>{csvError ? <span id="result-csv-error" style={{ color: 'var(--color-error)' }}>{csvError}</span> : null}</label>{csvRows.length ? <div role="status" style={{ padding: 12, borderRadius: 10, background: 'var(--color-success-soft)' }}>{csvRows.length} valid rows ready to upload.</div> : null}<Feedback message={action.message} error={action.error} /><Button onClick={upload} disabled={action.busy || !csvRows.length || !selectedEvent?.isOpen}>{action.busy ? 'Validating and saving…' : selectedEvent?.isOpen ? `Upload ${csvRows.length || ''} result rows`.trim() : 'Reopen entry to upload'}</Button><div className="result-danger-zone"><div><strong>Delete unpublished event</strong><p>Published results are protected and cannot be deleted.</p></div>{deleteArmed ? <div className="result-event-actions"><Button variant="outline" onClick={() => setDeleteArmed(false)}>Cancel</Button><Button variant="danger" onClick={deleteEvent}>Confirm delete</Button></div> : <Button variant="danger" onClick={() => setDeleteArmed(true)}>Delete event</Button>}</div></div> : <div role="status" style={{ padding: 24, textAlign: 'center' }}><h3>No result events yet</h3><p style={{ color: 'var(--text-muted)' }}>Create the first event above to generate its class roster.</p></div>}</Card>
  </Page>;
}
