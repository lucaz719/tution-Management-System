...
export function BranchResultsView() {
  ...
  const publishEvent = () => { if (!selectedEvent) return; void action.run(async () => {
    const result = await api.branchAdmin.publishResultDefinition(selectedEvent.id);
    await load(branchId);
    setReport(await api.branchAdmin.getResultReport(selectedEvent.id));
    return result;
  }, (result) => `${result.published} result${result.published === 1 ? '' : 's'} published to students and parents.`); };
  ...
  return <Page title="Branch results" description="Create an event, use its protected student roster CSV, then preview the standard printable report.">
    ...
    <Card hoverable={false}>
      <h2 style={{ fontSize: 18 }}>2. Add, review and publish marks</h2>
      <p style={{ color: 'var(--text-muted)', margin: '6px 0 16px' }}>Choose an event and follow the four steps. Students see only rows marked Published.</p>
      {data.resultDefinitions.length ? <div style={form}>
        <label style={label} htmlFor="result-event">Result event<select id="result-event" style={field} value={selectedId} onChange={(event) => { setSelectedId(event.target.value); setCsvRows([]); setCsvError(''); setDeleteArmed(false); setReport(null); }}>{data.resultDefinitions.map((item: any) => <option key={item.id} value={item.id}>{item.title} · {item.subject} · {new Date(item.testDate).toLocaleDateString()}</option>)}</select></label>
        {selectedEvent ? <>
          ...
          <section className="result-workflow-section" aria-labelledby="result-upload-heading">
          ...
          <section className="result-workflow-section" aria-labelledby="result-review-heading">
            ...
            <div className="result-event-actions"><Button variant="outline" disabled={!savedCount || action.busy} onClick={() => void reviewResults(selectedId)}><span className="material-symbols-outlined" aria-hidden="true">visibility</span>{savedCount ? `Review ${savedCount} saved result${savedCount === 1 ? '' : 's'}` : 'No saved marks to review'}</Button><Button disabled={!draftCount || action.busy} onClick={publishEvent}><span className="material-symbols-outlined" aria-hidden="true">publish</span>{draftCount ? `Publish ${draftCount} result${draftCount === 1 ? '' : 's'}` : 'All saved results published'}</Button></div>
            {report ? <div className="result-review-table"><div className="result-review-heading"><div><strong>{report.event.title}</strong><p>{report.event.gradeName} · {report.event.className} · {report.event.subject}</p></div><StatusBadge variant="info">{report.results.length} rows</StatusBadge></div><div className="accountant-table-scroll"><table className="accountant-table"><thead><tr><th>#</th><th>Admission no.</th><th>Student</th><th>Marks</th><th>Outcome</th><th>Visibility</th></tr></thead><tbody>{report.results.map((item: any, index: number) => <tr key={item.id}><td>{index + 1}</td><td>{item.admissionNumber || '—'}</td><td>{item.studentName}</td><td className="is-amount">{item.score} / {item.maximum}</td><td><StatusBadge variant={item.score >= item.passMarks ? 'success' : 'warning'}>{item.score >= item.passMarks ? 'Pass' : 'Needs improvement'}</StatusBadge></td><td><StatusBadge variant={item.published ? 'success' : 'info'}>{item.published ? 'Published' : 'Not published'}</StatusBadge></td></tr>)}</tbody></table></div></div> : null}
          </section>
      ...
      </div> : <div role="status" style={{ padding: 24, textAlign: 'center' }}><h3>No result events yet</h3><p style={{ color: 'var(--text-muted)' }}>Create the first event above to generate its class roster.</p></div>}
    </Card>
  </Page>;
}
...
function LegacyManagedTimetableView() {
  ...
  const submitSlot = (event: FormEvent) => { event.preventDefault(); if (slotForm.startTime >= slotForm.endTime) { setError('End time must be later than start time.'); return; } const next = { ...slotForm, room: slotForm.room.trim() }; const duplicate = slots.some((slot, index) => index !== editingIndex && slot.day === next.day && slot.startTime === next.startTime && slot.endTime === next.endTime && slot.room.toLowerCase() === next.room.toLowerCase()); if (duplicate) { setError('That weekly session is already listed.'); return; } setError(''); setSlots((current) => editingIndex === null ? [...current, next] : current.map((slot, index) => index === editingIndex ? next : slot)); resetSlot(); };
