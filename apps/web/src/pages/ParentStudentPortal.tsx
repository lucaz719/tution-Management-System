...
function AppointmentsView({ child, refresh }: { child: ParentChild; refresh: () => void }) {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (isGroup && participantIds.length === 0) return setStatus('Choose at least one additional participant for a group meeting.');
    setSubmitting(true);
    try { await requestAppointment({ studentId: child.id, teacherId, scheduledTime: new Date(scheduledAt).toISOString(), remarks, isGroup, participantIds }); setStatus('Appointment requested.'); setScheduledAt(''); setRemarks(''); setParticipantIds([]); refresh(); }
    catch (error) { setStatus(errorMessage(error)); } finally { setSubmitting(false); }
  };
  const minimumDateTime = new Date(Date.now() + bookingWindowHours * 3600000).toISOString().slice(0, 16);
  const otherTeachers = teachers.filter((teacher) => teacher.id !== teacherId);
  return <div className="parent-view"><section className="parent-card"><SectionHeader title="Appointment history" description={`Requests and linked negotiations for ${child.name}.`} />{appointments.length ? <div className="parent-appointment-list">{appointments.map((appointment) => <article key={appointment.id}><div><ParentStatus label={appointment.state} tone={toneForAppointment(appointment.state)} iconName={appointment.state === 'Alternative proposed' ? 'swap_horiz' : 'event'} /><h3>{appointment.teacher}</h3><p>{appointment.subject} · {appointment.group ? 'Group meeting' : 'Individual appointment'}</p></div><dl><div><dt>Requested</dt><dd>{appointment.requestedTime}</dd></div>{appointment.alternativeTime ? <div><dt>Linked alternative</dt><dd>{appointment.alternativeTime}</dd></div> : null}</dl></article>)}</div> : <EmptyState title="No appointments" message="New requests will appear here." iconName="event" />}</section><section className="parent-card"><SectionHeader title="Request an appointment" description={`${bookingWindowHours}-hour minimum booking window.`} /><form className="parent-form" onSubmit={(event) => void submit(event)}><label htmlFor="parent-appointment-teacher">Primary teacher</label><select id="parent-appointment-teacher" value={teacherId} onChange={(event) => { setTeacherId(event.target.value); setParticipantIds((ids) => ids.filter((id) => id !== event.target.value)); }} required>{teachers.map((teacher) => <option key={teacher.id} value={teacher.id}>{teacher.name} · {teacher.subject}</option>)}</select><label htmlFor="parent-appointment-type">Meeting type</label><select id="parent-appointment-type" value={isGroup ? 'group' : 'individual'} onChange={(event) => { const group = event.target.value === 'group'; setIsGroup(group); if (!group) setParticipantIds([]); }}><option value="individual">Individual</option><option value="group">Group — every participant must approve</option></select>{isGroup ? <fieldset className="parent-participant-fieldset"><legend>Additional participants <span aria-hidden="true">*</span></legend><p>Only teachers assigned to {child.name} can be invited.</p>{otherTeachers.length ? <div>{otherTeachers.map((teacher) => <label key={teacher.id}><input type="checkbox" checked={participantIds.includes(teacher.id)} onChange={(event) => setParticipantIds((ids) => event.target.checked ? [...ids, teacher.id] : ids.filter((id) => id !== teacher.id))} /><span>{teacher.name}<small>{teacher.subject}</small></span></label>)}</div> : <small>No additional assigned teachers are available.</small>}</fieldset> : null}<label htmlFor="parent-appointment-time">Date and time</label><input id="parent-appointment-time" type="datetime-local" min={minimumDateTime} value={scheduledAt} onChange={(event) => setScheduledAt(event.target.value)} aria-invalid={status.includes('hours in advance') ? 'true' : undefined} aria-describedby={status ? 'parent-appointment-status' : undefined} required /><label htmlFor="parent-appointment-reason">Reason</label><textarea id="parent-appointment-reason" value={remarks} onChange={(event) => setRemarks(event.target.value)} required placeholder={`For example: discuss ${child.name}’s progress`} />{status ? <p id="parent-appointment-status" className={status === 'Appointment requested.' ? 'parent-form__notice' : 'parent-form__error'} role={status === 'Appointment requested.' ? 'status' : 'alert'}>{status}</p> : null}<button className="parent-primary-button" type="submit" disabled={!teachers.length || submitting} aria-busy={submitting}>{icon('event_upcoming')}{submitting ? 'Requesting…' : 'Request appointment'}</button></form></section></div>;
}
