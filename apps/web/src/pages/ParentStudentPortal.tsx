...
function FeesView({ child }: { child: ParentChild }) {
  ...
  return <div className="parent-view"><section className="parent-fee-hero"><div><ParentStatus label={child.blocked ? 'Blocked' : current.state} tone={child.blocked ? 'gold' : toneForInvoice(current.state)} iconName={child.blocked ? 'lock' : 'payments'} /><span>{current.state === 'Paid' ? 'Latest payment for' : 'Current payable for'} {child.name}</span><strong>{money(invoiceTotal(current))}</strong><p>{current.invoiceType === 'ADMISSION' ? 'One-time admission fee' : current.cycle}{current.state === 'Paid' && current.paymentDate ? ` · Paid ${current.paymentDate}` : ` · Due ${current.dueDate}`}</p></div><button ref={triggerRef} type="button" onClick={() => current.qrAvailable ? setSelected(current) : setBill(current)}>{icon(current.qrAvailable ? 'qr_code_2' : 'receipt_long')}{current.qrAvailable ? 'Show Nepal Pay QR' : 'View paid bill'}</button></section><section className="parent-card"><SectionHeader title="Payment history" description={`Admission and monthly invoices for ${child.name}.`} /><div className="parent-payment-calendar">{invoices.map((invoice) => <article key={invoice.id} className={`is-${toneForInvoice(invoice.state)}`}><div><strong>{invoice.invoiceType === 'ADMISSION' ? 'Admission fee' : invoice.cycle}</strong><span>{invoice.state === 'Paid' && invoice.paymentDate ? `Paid ${invoice.paymentDate}` : `Due ${invoice.dueDate}`}</span></div><ParentStatus label={invoice.state} tone={toneForInvoice(invoice.state)} iconName={invoice.state === 'Paid' ? 'check_circle' : invoice.state === 'Overdue' ? 'error' : 'schedule'} /><b>{money(invoiceTotal(invoice))}</b><div className="parent-invoice-actions"><button type="button" onClick={() => setBill(invoice)}>View bill</button>{invoice.qrAvailable ? <button type="button" onClick={() => setSelected(invoice)}>Open QR</button> : null}</div></article>)}</div></section><div className="parent-fees-layout"><section className="parent-card"><SectionHeader title={current.invoiceType === 'ADMISSION' ? 'Admission invoice' : `${current.cycle} invoice`} description={current.invoiceType === 'ADMISSION' ? 'One-time admission payment. This is not monthly tuition.' : 'Itemized monthly billing-cycle breakdown.'} /><div className="parent-invoice-lines">{current.lines.map((line) => <div key={line.label}><span>{line.label}</span><strong className={line.amount < 0 ? 'is-discount' : ''}>{line.amount < 0 ? '−' : ''}{money(line.amount)}</strong></div>)}<div className="parent-invoice-total"><span>Net payable</span><strong>{money(invoiceTotal(current))}</strong></div></div></section><aside className="parent-card parent-payment-help">{icon('verified_user')}<h3>Payment details</h3><p>{current.state === 'Paid' ? 'This invoice is fully paid.' : 'Confirm the selected child and exact invoice details.'}</p><dl><div><dt>Child</dt><dd>{child.name}</dd></div><div><dt>Reference</dt><dd>{current.reference}</dd></div></dl></aside></div>{selected ? <PaymentDialog invoice={selected} child={child} onClose={() => { setSelected(null); window.setTimeout(() => triggerRef.current?.focus(), 0); }} /> : null}{bill ? <InvoiceDocumentDialog data={bill.document} onClose={() => setBill(null)} /> : null}</div>;
}
