...
void main() {
  group('AppValidators tests', () {
    test('isValidEmail accepts valid emails', () {
      ...
      expect(AppValidators.isValidEmail('teacher.shyam@pinnacle.edu.np'), isTrue);
  ...
  group('Formatter utility tests', () {
    test('maskEmail obfuscates emails properly', () {
      ...
      expect(maskEmail('a@b.com'), 'a*@b.com'); // boundary case (parts.first.length <= 1)
      expect(maskEmail('ab@cd.com'), 'a*@cd.com'); // boundary case (parts.first.length == 2)
      expect(maskEmail('abc@def.com'), 'ab*@def.com'); // boundary case (parts.first.length == 3)
    ...
    test('formatDurationClock outputs HH:MM:SS', () {
      expect(formatDurationClock(const Duration(hours: 1, minutes: 2, seconds: 3)), '01:02:03');
      expect(formatDurationClock(const Duration(minutes: 45, seconds: 12)), '00:45:12');
